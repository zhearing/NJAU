...
void CSnapDlg::OnBnClickedButtonStart()
{
	...
	if (center.y != 0)
	{
		...
		Sleep(1000); Sleep(1000); Sleep(300);
		//Sleep(500);// Sleep(100);// Sleep(550);
		//Sleep(250); //Sleep(50);
		Grab200();
		center.y = 0;
		UpdateData(FALSE);
...
int CSnapDlg::Rgb2Hsi(const IplImage* src, IplImage* dataH, IplImage* dataS, IplImage* dataI)
{
	//返回0：成功，1：失败
...
void CSnapDlg::extractyellow()
{
	IplImage *src;//定义源图像指针
	IplImage *dst_contours;//定义轮廓图像指针

	IplConvKernel *element = 0;//定义形态学结构指针

	int Num_Object = 0; //定义目标对象数量
	...
	int Nc = cvFindContours(dst_contours, storage, &first_contour, sizeof(CvContour), CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE);
	cvZero(dst_contours);   //初始化图像空间
	...
	for (c = first_contour; c != NULL; c = c->h_next)
	{
		...
		if (l > la&& l < 700)
		{
			cvZero(dst_contours);		//单独将每个轮廓画出
			cvDrawContours(dst_contours, c, cvScalarAll(255), cvScalarAll(0), 0, -1, 8);   //CV_FILLED即负数表示画轮廓时同时填充内部

			//根据每个轮廓提取出每一个目标,背景为黑色
			cvZero(ea_tar);
			cvCopy(img1, ea_tar, dst_contours);    //掩膜
	...
	/*cvReleaseImage(&src);
	cvReleaseImage(&ea_tar);
	cvReleaseImage(&ea_tar1);
	cvReleaseImage(&dst_contours);
	cvReleaseImage(&HImg);
	cvReleaseImage(&SImg);
	cvReleaseImage(&IImg);
	cvReleaseImage(&img_LAB);
	cvReleaseImage(&channel_L);
	cvReleaseImage(&channel_A);
	cvReleaseImage(&channel_B);
	cvReleaseImage(&m_dest);
	cvReleaseImage(&m_destRGB);
	cvReleaseImage(&thre_L);
	cvReleaseImage(&thre_A);
	cvReleaseImage(&thre_B);
	cvReleaseImage(&img);
	cvReleaseImage(&img1);*/
...
void CSnapDlg::Grab25()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command2_5[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab50()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command5[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab75()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command7_5[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab100()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command10[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab125()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command12_5[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab150()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command15[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab175()
{	
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command17_5[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}

void CSnapDlg::Grab200()
{
	int i = 0;
	for (i = 0; i < 7; i++)
	{
		m_strTXData = command20[i];
		m_ctrlComm.put_Output(COleVariant(m_strTXData)); //发送数据
		Sleep(500);
	}
}
...
int CSnapDlg::cac_delay(int x)
{
	double delay = 0;
	int delay_time = 0;
	int err = 700;
	delay = (-0.1327*x + 7.8179)*1000;
	delay_time = int(delay) + err;
	return delay_time;
}
